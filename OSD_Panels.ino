...
void panLogo(){
    osd.setPanel(5, 5);
    osd.openPanel();
    osd.printf_P(PSTR("MinimOSD-Extra 2.4|modded by dispetto and lothar"));
    osd.closePanel();
}
...
void panCALLSIGN(int first_col, int first_line){
    ...
    osd.printf("retl-guid-%cmanu%c-stab-trai", 0x7B, 0x7D) ; }
      if(flight_mode_selected == 1){ 
    osd.printf("guid-manu-%cstab%c-trai-fbwa", 0x7B, 0x7D); }
      if(flight_mode_selected == 2){ 
    osd.printf("manu-stab-%ctrai%c-fbwa-fbwb", 0x7B, 0x7D); }
      if(flight_mode_selected == 3){ 
    osd.printf("stab-trai-%cfbwa%c-fbwb-circ", 0x7B, 0x7D); }
      if(flight_mode_selected == 4){ 
    osd.printf("trai-fbwa-%cfbwb%c-circ-loit", 0x7B, 0x7D); }
      if(flight_mode_selected == 5){ 
    osd.printf("fbwa-fbwb-%ccirc%c-loit-auto", 0x7B, 0x7D); }
      if(flight_mode_selected == 6){ 
    osd.printf("fbwb-circ-%cloit%c-auto-retl", 0x7B, 0x7D); }
      if(flight_mode_selected == 7){ 
    osd.printf("circ-loit-%cauto%c-retl-guid", 0x7B, 0x7D); }
      if(flight_mode_selected == 8){ 
    osd.printf("loit-auto-%cretl%c-guid-manu", 0x7B, 0x7D); }
      if(flight_mode_selected == 9){ 
    osd.printf("auto-retl-%cguid%c-manu-stab", 0x7B, 0x7D); }
    osd.closePanel();
}
